...
async function generateContent() {
  ...
  // Load life
  try {
    const lifeDir = resolve(contentPath, "life");
    const lifeFiles = await readdir(lifeDir);
    for (const file of lifeFiles) {
      if (file.endsWith(".yaml") || file.endsWith(".yml")) {
        const filePath = resolve(lifeDir, file);
        const fileStat = await stat(filePath);
        if (fileStat.isFile()) {
          const content = await readFile(filePath, "utf-8");
          const life = yaml.load(content) as any;
          const slug = file.replace(/\.(yaml|yml)$/, "");
          life.slug = slug;
          data.life.push(life);
        }
    ...
    console.warn("Failed to load life:", error);
  }
    ...
    if (photosData.photos) {
      ...
      for (const photo of photosData.photos) {
        if (photo.url && !photo.url.startsWith("/attached_assets/")) {
          console.warn(`警告: 照片 "${photo.caption}" 的 URL "${photo.url}" 應該以 "/attached_assets/" 開頭`);
        }
      }
  ...
  console.log(`  - Life: ${data.life.length}`);
